...
import { SpotifyService, Track } from './spotify.service';
...
export class TrackPlayingService implements OnDestroy {
  private playingStates = new Map<string, TrackPlayingState>();
  private destroy$ = new Subject<void>();

  constructor(private spotifyService: SpotifyService) {
    this.subscribeToAudioPlayerState();
  }
  ...
  private subscribeToAudioPlayerState(): void {
    // Subscribe to the actual audio player state changes
    this.spotifyService.currentTrack$
      .pipe(takeUntil(this.destroy$))
      .subscribe((currentTrack) => {
        // Clear all playing states when track changes
        this.clearAllPlayingStates();

        if (currentTrack) {
          // Set the new current track as playing
          this.setPlayingState(currentTrack.id, {
            isPlaying: true,
    ...
    this.spotifyService.isPlaying$
      .pipe(takeUntil(this.destroy$))
      .subscribe((isPlaying) => {
        // Update all playing states based on actual audio player state
        this.playingStates.forEach((state, trackId) => {
          if (state.isPlaying !== isPlaying) {
            this.setPlayingState(trackId, {
              ...state,
              isPlaying: isPlaying,
            });
          }
  ...
  async playTrack(
    ...
    try {
      ...
      // Clear searching state - the actual playing state will be set by the audio player subscription
      this.setPlayingState(track.id, {
        isPlaying: false,
        currentTrackId: track.id,
        isSearchingPreview: false,
   ...
   */
  getCurrentPlayingTrackId(): string | null {
    for (const [trackId, state] of this.playingStates.entries()) {
      if (state.isPlaying) {
        return trackId;
      }
    }
